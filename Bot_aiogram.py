...
def get_keyboard() -> types.InlineKeyboardMarkup:
    buttons = [
        ...
        types.InlineKeyboardButton(text="Выбор группы", callback_data="day_newgroup")
    ]
...
async def send_welcome(message: types.Message):
    await message.answer("Приветствую! Это бот для отслеживания расписания."
...
async def timetable(message: types.Message):
    await message.answer("Напишите группу у которой хотите посмотреть расписание:\nПример: М4О-307Б-20")
    ...
    with suppress(MessageNotModified):
        ...
        if action == "today":
            ...
            await call.message.edit_text(
                ...
                f'\nhttps://my.mai.ru/ - Ссылка на личный кабинет МАИ'
                f'\nhttps://lms.mai.ru/ - Ссылка на LMS МАИ',
                reply_markup=get_keyboard_info(), parse_mode='Markdown', disable_web_page_preview=True)
        elif action == "newgroup":
            await call.message.edit_text("Напишите группу у которой хотите посмотреть расписание:")
            await UserState.group.set()
        elif 'None' in action:
            try:
